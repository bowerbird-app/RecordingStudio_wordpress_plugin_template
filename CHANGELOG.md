...
### Upgrade notes
- Rebuild and reinstall the plugin ZIP (`bin/build-plugin-zip` or `npm run build` under `wordpress/recording-studio-widgets`) so Connect no longer jumps straight to the host. PKCE, token storage, and the host allowlist are unchanged.
