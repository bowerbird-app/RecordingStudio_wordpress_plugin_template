...
- **Settings → WordPress Plugin Demo**: **Login** and **Register** start Connect (PKCE through the Oauth 0.4.0 central relay). Connected state keeps **Disconnect** and **Connect again**. Advanced is a dropdown for **Connect via API key** (API key and Secret key). **Save settings** and **Test connection** sit inside that dropdown. Host URL and client id are baked (`CloudHost`).
...
9. To ship a flavored ZIP, edit `wordpress/recording-studio-widgets/includes/RecordingStudio/ProductConfig.php` before the build. `NAME` is the Settings menu label and page heading. `OAUTH_CONNECT` shows Login and Register when disconnected, and Disconnect plus Connect again when connected. `LOGIN_BUTTON_TEXT` and `REGISTER_BUTTON_TEXT` are those disconnected labels. `API_KEYS` shows Advanced (API key, Secret key, Save settings, and Test connection). When `API_KEYS` is false, Save and Test are absent. Leave both flags true for WordPress Plugin Demo. Site owners do not edit this file. Host URL and client id overrides stay `RECORDING_STUDIO_HOST_BASE_URL` and `RECORDING_STUDIO_CLIENT_ID` in wp-config.
...
3. Click **Login** or **Register**, then log in and approve.
...
| Oauth | dummy GitHub tag `v0.4.2` (central relay; one public WordPress app; consent is a full page) |
