...
The plugin in `wordpress/recording-studio-widgets/` is a dynamic block named **WP Template Demo**. It stores Connect tokens or API keys server-side, fetches BrowserPayload v1 JSON from the dummy host named API `wp_plugin_demo`, server-renders the payload, and mounts the baked plugin SDK on the front. `wp-env` serves WordPress on port 8888. The dummy host serves Rails on port 3000.

### Phase 4 (WordPress Plugin Demo client)

- **Settings → WP Template Demo**: **Login** is the primary button and starts Connect (PKCE through the Oauth central relay). **Register** is the secondary button. It opens the host signup page in a new tab when `ProductConfig::REGISTER` is true and `GET {host}/recording_studio_oauth/connect/options?client_id={client id}` returns `registration: true` with a `registration_url`. Connected state keeps **Disconnect** and **Connect again**. The heading shows `ProductConfig::LOGO` before the title. Advanced is hidden while `ProductConfig::API_KEYS` is false. Set that flag to true and rebuild to show **Connect via API key**, the API key, the Secret key, **Save settings**, and **Test connection**. Host URL and client id are baked (`CloudHost`). The block icon is the same logo file (`build/brand/product-logo-red.jpg`). The block description is `ProductConfig::DESCRIPTION`. The sidebar lists page titles from the host `title` field.
...
9. To ship a flavored ZIP, edit `wordpress/recording-studio-widgets/includes/RecordingStudio/ProductConfig.php` before the build. `NAME` is the Settings menu label, the Settings heading, and the block title. `DESCRIPTION` is the sentence under the block title. `OAUTH_CONNECT` shows Login when disconnected, and Disconnect plus Connect again when connected. `REGISTER` shows Register when the host also allows signup. `LOGIN_BUTTON_TEXT` and `REGISTER_BUTTON_TEXT` are those disconnected labels. `LOGO` is the block icon and the image before the Settings heading (`build/brand/product-logo-red.jpg`). Replace `assets/brand/product-logo-red.jpg` and rebuild when a flavor needs a different icon. `API_KEYS` shows Advanced (API key, Secret key, Save settings, and Test connection). When `API_KEYS` is false, that section is absent. The demo ships `API_KEYS` false and `REGISTER` true. Set `API_KEYS` to true and rebuild when a flavor should show Advanced again. Site owners do not edit this file. Host URL and client id overrides stay `RECORDING_STUDIO_HOST_BASE_URL` and `RECORDING_STUDIO_CLIENT_ID` in wp-config.
...
2. Open **Settings → WP Template Demo**.
...
| Oauth | dummy GitHub tag `v0.5.0` (central relay; connect options; one public WordPress app; consent is a full page) |
