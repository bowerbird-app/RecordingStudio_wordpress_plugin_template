...
9. To ship a flavored ZIP, edit `wordpress/recording-studio-widgets/includes/RecordingStudio/ProductConfig.php` before the build. `NAME` is the Settings menu label, the Settings heading, and the block title. `DESCRIPTION` is the sentence under the block title. `OAUTH_CONNECT` shows Login when disconnected, and Disconnect plus Connect again when connected. `LOGIN_BUTTON_TEXT` is the disconnected Login label. `LOGO` is the block icon and the image before the Settings heading (`build/brand/product-logo-red.jpg`). Replace `assets/brand/product-logo-red.jpg` and rebuild when a flavor needs a different icon. `API_KEYS` shows Advanced (API key, Secret key, Save settings, and Test connection). When `API_KEYS` is false, that section is absent. The demo ships `API_KEYS` false. Set `API_KEYS` to true and rebuild when a flavor should show Advanced again. Site owners do not edit this file. Host URL and client id overrides stay `RECORDING_STUDIO_HOST_BASE_URL` and `RECORDING_STUDIO_CLIENT_ID` in wp-config.
...
3. Click **Login**, then log in and approve.
