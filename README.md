...
9. To ship a flavored ZIP, edit `wordpress/recording-studio-widgets/includes/RecordingStudio/ProductConfig.php` before the build. `NAME` is the Settings menu label and page heading. `OAUTH_CONNECT` shows Connect, Disconnect, and Connect again. `API_KEYS` shows Advanced (API key and Secret key). Leave both true for WordPress Plugin Demo. Site owners do not edit this file. Host URL and client id overrides stay `RECORDING_STUDIO_HOST_BASE_URL` and `RECORDING_STUDIO_CLIENT_ID` in wp-config.
