...
class RecordingStudioWordpressPluginTemplateTest < Minitest::Test
  ...
  def test_dummy_gemfile_pins_verified_4x_github_tags
    ...
    assert_includes gemfile, 'github: "bowerbird-app/RecordingStudio_Oauth", tag: "v0.5.1"'
    lockfile = File.read(File.expand_path("dummy/Gemfile.lock", __dir__))
    assert_includes lockfile, "recording_studio_oauth (0.5.1)"
    initializer = File.read(File.expand_path("dummy/config/initializers/recording_studio_oauth.rb", __dir__))
    assert_includes initializer, "config.allow_registration = false"
    schema = File.read(File.expand_path("dummy/db/schema.rb", __dir__))
    refute_includes schema, "recording_studio_oauth_registration_settings"
  ...
  def test_product_readme_is_the_template_guide
    ...
    assert_includes readme, "dummy GitHub tag `v0.5.1` (central relay"
