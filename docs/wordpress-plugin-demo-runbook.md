...
Preferred for minting WordPress API client credentials: sign in to the dummy and open sidebar **API Keys** (`/recording_studio_api/api_clients`). Preferred for staff OAuth apps: sidebar **Registered apps** (`/admin/screens/oauth_clients`). For the named API `wp_plugin_demo` client used by the plugin block, the console print below still works.
