...
Product UI name is **WP Template Demo**. The feature is the WordPress plugin feature. Do not call it Featured In.
...
### Settings → WP Template Demo

The screen is a card form styled only on that page, and the WordPress sidebar and top bar stay as they are.

1. Click **Login** or **Register**. There is no host URL field and no client id field. Both buttons start the same Connect flow.
...
Use this only when you want client credentials instead of Connect. Open **Advanced** and use **Connect via API key**. Advanced has **API key**, **Secret key**, **Save settings**, and **Test connection**. The host is the same baked cloud host. When `API_KEYS` is false, Advanced and those buttons are absent. The demo ZIP ships `API_KEYS` false. Set it to true and run `bin/build-plugin-zip` again to bring the section back.
...
4. Settings **Login** or **Register** completes after Users sign-in, or Advanced **Test connection** succeeds.
