...
Use this only when you want client credentials instead of Connect. Open **Advanced** and use **Connect via API key**. Advanced has **API key**, **Secret key**, **Save settings**, and **Test connection**. The host is the same baked cloud host. When `API_KEYS` is false, Advanced and those buttons are absent.
